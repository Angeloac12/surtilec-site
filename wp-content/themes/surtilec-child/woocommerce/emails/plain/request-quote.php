<?php
/**
 * YITH Request a Quote — admin email (plain). Surtilec override.
 *
 * Override of templates/emails/plain/request-quote.php: the price subtotal is
 * removed (quote catalog shows no prices). Keeps plugin hooks. Theme override,
 * not a plugin edit.
 *
 * @package Surtilec
 */
...
if ( ! empty( $raq_data['raq_content'] ) ) :
	foreach ( $raq_data['raq_content'] as $item ) :
		...
		echo esc_html__( 'Cantidad', 'surtilec' ) . ': ' . esc_html( $item['quantity'] );
		echo "\n";
	endforeach;
endif;
