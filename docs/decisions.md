...
- **Decision — permalinks:** product base `producto`, **category base `categoria`**, attribute base `filtro`. (Originally tried category base `productos`, but it **collides with the shop page slug `productos`** — under `/%postname%/` WordPress uses verbose page rules, so the page's rules capture `/productos/<child>/` as a child-page lookup and real requests 404. Switched to `categoria`, verified all category/product URLs return real 200.) Site is pre-indexing, so no redirects added; internal links are dynamic (`get_term_link`/`get_permalink`).
  - **Verification lesson:** with Coming Soon ON, anonymous `curl` returns 200 for **every** front URL (the coming-soon page intercepts), masking real 404s; and regenerating rules in `wp eval` and regex-matching the category rule gives a **false positive** because it ignores verbose page-rule precedence. To verify real URL resolution, briefly toggle `woocommerce_coming_soon` off (restore immediately) or test logged-in.
